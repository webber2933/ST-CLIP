...
from STCLIP.dataset.datasets.evaluation import evaluate
from STCLIP.utils.comm import get_rank, is_main_process, all_gather, gather, synchronize, get_world_size
...
def compute_on_dataset_1stage(model, data_loader, device):
    ...
    for batch in tqdm(data_loader, **extra_args):
        ...
        objects = [None if (box is None) else box.to(device) for box in keypoints]
...
def compute_on_dataset_2stage(model, data_loader, device, logger):
    ...
    for i, batch in enumerate(tqdm(data_loader, **extra_args)):
        ...
        batch_info_list[i] = (movie_ids, timestamps, video_ids, boxes, [b.extra_fields['det_score'] for b in boxes])
    ...
    start_time = time.time()
    for movie_ids, timestamps, video_ids, boxes, det_scores in tqdm(batch_info_list, **extra_args):
        extras = dict(
            movie_ids=movie_ids,
            timestamps=timestamps,
        )
        with torch.no_grad():
            output = model(None, None, boxes, None, extras=extras, part_forward=1)
            output = [o.to(cpu_device) for o in output]
    ...
    del batch_info_list, output, extras, movie_ids, timestamps, video_ids
    return results_dict
...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu):
    ...
    if len(video_ids) != video_ids[-1] + 1:
        logger = logging.getLogger("STCLIP.inference")
        logger.warning(
